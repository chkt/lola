...
class HttpRequest {
	 ...
	 * Returns an instance representing the origin request
	...
	/**
	 * Returns the value of header param $attr
	 * @param string $raw The header string
	 * @param string $attr The header attribute
	 * @return string
	 */
	...
	/**
	 * Returns a sorted list of values of header
	 * @param string $attr The header string
	 * @param string $pattern The header value pattern
	 * @return string[]
	 * @throws \ErrorException if any header value is invalid
	 */
	...
	/**
	 * Returns the origin header referenced by $name
	 * @param string $name The header name
	 * @return string
	 */
	static public function originHeader($name) {
		$origin =& self::$_origin;

		if (!array_key_exists('headers', $origin)) $origin['headers'] = [];
		...
		if (!array_key_exists($name, $headers)) {
			...
			$header = filter_input(INPUT_SERVER, 'HTTP_' . $filter);
	...
	public function getHeader($name) {
		return array_key_exists($name, $this->_header) ? $this->_header[$name] : self::originHeader($name);
	}
	 ...
	 * @throws \ErrorException if $value is no a string
	 */
	public function setHeader($name, $value) {
		if (!is_string($name) || empty($name) || !is_string($value)) throw new \ErrorException();
